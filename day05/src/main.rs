...
fn parse(input: &str) -> (petgraph::Graph<u32, ()>, Vec<Vec<u32>>){
    ...
    (parse_page_ordering_rules(page_ordering_input), parse_update_pages(update_pages_input))
}

fn parse_page_ordering_rules(input: &str) -> petgraph::Graph<u32, ()> {
    let edges: Vec<(u32, u32)> = input.lines().map(|line| {
        line.split_once("|").map(|(a_str, b_str)| {
...
fn parse_update_pages(input: &str) -> Vec<Vec<u32>> {
    input.lines().map(|line| {line.split(',').filter_map(|s| s.parse::<u32>().ok()).collect()}).collect()
}

fn is_sorted(data: &[u32], graph: &petgraph::Graph<u32, ()>, graph_indices: &HashMap<u32, NodeIndex>) -> bool {
    data.windows(2).all(|w| compare(w[0], w[1], graph, graph_indices) == Ordering::Less)
}

fn compare(a: u32, b: u32, graph: &petgraph::Graph<u32, ()>, graph_indices: &HashMap<u32, NodeIndex>) -> Ordering {
