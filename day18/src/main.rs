...
fn parse_bytes(input: &str) -> Vec<utils::coord::Coord> {
    input.lines().map(|line| {
        let (x,y) = line.split_once(",").map_or((0, 0), |(x, y)| (x.parse().unwrap(), y.parse().unwrap()));
        Coord::new(x,y)
    }
    ).collect()
}

fn create_graph(matrix_size: i32) -> (petgraph::Graph<Coord, i32>, HashMap<Coord, petgraph::graph::NodeIndex>, petgraph::graph::NodeIndex, petgraph::graph::NodeIndex) {
    let mut graph = petgraph::Graph::new();
    let mut nodes = HashMap::new();

    let start_node = graph.add_node(Coord { x: 0, y: 0 });
    let end_node = graph.add_node(Coord { x: matrix_size-1, y: matrix_size-1 });
...
fn part_one(input: &str, matrix_size: i32, first_n_bytes: usize) -> i32 {
    ...
    graph.retain_nodes(|graph, node_index| {
        let node_coord = graph.node_weight(node_index).expect("node should be present");
...
fn main() {
    ...
    println!("{} part one: {}", env!("CARGO_PKG_NAME"), part_one(INPUT, 71, 1024));
    println!("{} part two: {:?}", env!("CARGO_PKG_NAME"), part_two(INPUT, 71));
}
