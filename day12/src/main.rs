    ...
    fn new(input: &str) -> Self {
        ...
        matrix.iter().enumerate().for_each(|(i, row)| {
            row.iter().enumerate().for_each(|(j, char)| {
                node_indices.insert(Coord(i as i32, j as i32), graph.add_node((*char, Coord(i as i32, j as i32))));
            })
        });
        ...
        for i in 0..matrix.len() {
            for j in 0..matrix[i].len() {
                ...
                for neighbour in get_neighbours(coord) {
                    ...
                    let (neighbour_crop_type, _) = *graph.node_weight(*neighbour_node_id.unwrap()).unwrap();
...
fn part_two(input: &str) -> u64 {
    ...
    for crop_field in crop_fields {
        ...
        let mut trickies = 0;
        let coords_of_crop_field = crop_field.iter().map(|c| map.graph.node_weight(*c).unwrap().1).collect::<Vec<_>>();
        for crop_node_index in crop_field {
            ...
            get_corners(crop_node_coord).iter().for_each(|c| {
                ...
                let breaks = match (is_0_crop, is_1_crop, is_2_crop) {
                    ...
                    (false, true, false) => { trickies += 1; true }
                    (_, _, _) => { true }
                };
